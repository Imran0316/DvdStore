...
namespace DvdStore.Controllers
{
    public class ProfileController : Controller
    {
        ...
        public async Task<IActionResult> OrderDetails(int id)
        {
            ...
            Console.WriteLine($"OrderDetails - OrderID: {id}, UserID: {userId}");

            // FIX: Use Include and ThenInclude properly
            var order = await _context.tbl_Orders
                .Include(o => o.tbl_OrderDetails)
                    .ThenInclude(od => od.tbl_Products)
                        .ThenInclude(p => p.tbl_Albums)  // Include Album for product
                .Include(o => o.tbl_OrderDetails)
                    .ThenInclude(od => od.tbl_Products)
                        .ThenInclude(p => p.tbl_Producers)  // Include Producer if needed
            ...
            Console.WriteLine($"Found order #{order.OrderID} with {order.tbl_OrderDetails?.Count} items");
            ...
            if (order.tbl_OrderDetails != null)
            {
                ...
                {
                    Console.WriteLine($"Item: {item.tbl_Products?.tbl_Albums?.Title}, Quantity: {item.Quantity}");
                }
            }
